...
def db_update_task(task_data):
    ''' update task description and status for task with id '''
    taskid = task_data['taskid']
    taskD = task_data['description']
    taskSt = 1 if task_data['status'] else 0;
    db = DaBa("todo")
    cmd = "UPDATE todo SET task = ?, status = ? WHERE id = ?"
    db.exec(cmd, (taskD, taskSt, taskid))
    db.close()
...
@route('/task/delete/<id>', method='POST')
def delete_task(id):
    ''' delete a task '''
    print("from delete_task: ")
    print(id, flush = True)
    db_delete_task(id)
    result = db_get_task(id)
    if result is None:
        return pack_task({ "result" : "true" })
    else:
        return pack_task({ "result": "false" })

@route('/edit/save', method='POST')
def save_edit_task():
    ''' save edit task '''
    # decode json message body into Python dictionary
    task_data = unpack_task(request)
    print("from save_edit_task: ")
    print(task_data)
    db_update_task(task_data)
    updated_task = view_get_task(task_data['taskid'])
    return pack_task(updated_task)
